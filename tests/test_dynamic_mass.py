...
class TwoSpeciesTestCase(unittest.TestCase):
    ...
    def test_exponentially_increase_to_steady_state_count_with_constant_mass(self):
        ...
        # simulate
        time, pop_constant, pop_dynamic, comp_mass, comp_vol = self.simulate(self.model, max_time=200.)
    ...
    def test_exponentially_increase_to_steady_state_mass_and_descend_to_concentration(self):
        ...
        * In submodel_degradation:
        ...
        # simulate
        max_time = 1000.
        time, pop_constant, pop_dynamic, comp_mass, comp_vol = self.simulate(self.model, max_time=max_time)
        ...
        numpy.testing.assert_allclose(list(comp_density.to_numpy()), [self.density.value]*int(max_time + 1))
        ...
        third_quarter = int((3 * max_time / 4) + 1)
...
class MetabolismAndGeneExpressionTestCase(unittest.TestCase):
    ...
    def test(self):
        ...
        simulation = Simulation(model)
        results_dirname = simulation.run(max_time=8 * 3600,
                                         ode_time_step=1.,
