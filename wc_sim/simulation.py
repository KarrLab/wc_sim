...
from de_sim.simulation_config import SimulationConfig as DESimConfig
...
class Simulation(object):
    ...
    Attributes:
        ...
        de_sim_config (:obj:`DESimConfig`): a DE-Sim simulation configuration
        simulation_engine (:obj:`SimulationEngine`): the `SimulationEngine`
    """
    def __init__(self, model, de_sim_config=None):
        """
        Args:
            model (:obj:`str` or `Model`): either a path to file(s) describing a `wc_lang` model, or
                a `Model` instance
            de_sim_config (:obj:`DESimConfig`, optional): a DE-Sim simulation configuration
    ...
    def _create_metadata(self):
        ...
        """
        # FIX FOR DE-SIM CHANGES: redundant with DE-Sim
        # model metadata
        ...
        # FIX FOR DE-SIM CHANGES: combine w DE-Sim
        if self.sim_config:
    ...
    def process_and_validate_args(self, args):
        ...
        # FIX FOR DE-SIM CHANGES: revise
        Supported arguments are results_dir, checkpoint_period, ode_time_step, and time_max. time_max is
        ...
        """
        # todo: FIX FOR DE-SIM CHANGES: remove checks which are redundant with SimulationConfig
        ...
        # validate de_sim_config
        self.de_sim_config.validate()

        # validation not provided by de_sim_config.validate()
        if self.de_sim_config.time_max <= 0:
            raise MultialgorithmError("End time ({self.de_sim_config.time_max}) must be positive")

        # validate de_sim_config and wc_sim_config together
        if 'checkpoint_period' in args:
            if args['checkpoint_period'] <= 0 or self.de_sim_config.time_max < args['checkpoint_period']:
                raise MultialgorithmError("Checkpointing period ({}) must be positive and "
                                          "less than or equal to time max".format(args['checkpoint_period']))

            if self.de_sim_config.time_max / args['checkpoint_period'] % 1 != 0:
                raise MultialgorithmError('time_max ({}) must be a multiple of checkpoint_period ({})'.format(
                                          self.de_sim_config.time_max, args['checkpoint_period']))

        # todo: FIX FOR DE-SIM CHANGES: add dFBA timestep
        if 'ode_time_step' in args:
            if args['ode_time_step'] <= 0.0 or self.de_sim_config.time_max < args['ode_time_step']:
    ...
    def run(self, time_max, results_dir=None, checkpoint_period=None, ode_time_step=1, seed=None,
            submodels_to_skip=None, progress_bar=True, verbose=True, options=None):
        """ Run one simulation
        ...
        """
        # FIX FOR DE-SIM CHANGES: move into from de_sim.simulation_config.SimulationConfig:
        self.de_sim_config = DESimConfig(time_max)
        self.de_sim_config.metadata_dir = results_dir
        self.de_sim_config.random_seed = seed
        self.de_sim_config.progress = progress_bar

        if seed is not None:
            RandomStateManager.initialize(seed=seed)

        self.sim_config = sim_config.SimulationConfig(time_max=time_max, ode_time_step=ode_time_step)
        self._prepare()
        ...
        # run simulation
        # FIX FOR DE-SIM CHANGES: pass a SimulationConfig
        try:
            num_events = self.simulation_engine.simulate(self.de_sim_config)
